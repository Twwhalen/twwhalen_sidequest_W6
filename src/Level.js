...
export class Level {
  ...
  _damageBoar(e, facingDir) {
    ...
    // Prefer tuning.boar.stats.hp, fall back to tuning.boar.hp, then default 3
    const defaultHp = Number(this.tuning?.boar?.stats?.hp ?? this.tuning?.boar?.hp ?? 3);
    e.hp = Math.max(0, (e.hp ?? defaultHp) - 1);
