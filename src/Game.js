...
export class Game {
  ...
  _wireEventListeners() {
    ...
    // -----------------------
    if (this.sound) {
      // Map game events to the SFX files present in assets/sfx/
      this._unsubs.push(this.events.on("leaf:collected", () => this.sound.play("leafCollect")));
      this._unsubs.push(this.events.on("player:damaged", () => this.sound.play("receiveDamage")));
      this._unsubs.push(this.events.on("player:died", () => this.sound.play("receiveDamage")));
      this._unsubs.push(this.events.on("boar:damaged", () => this.sound.play("hitEnemy")));
      // Play crowd cheer when a boar fully dies
      this._unsubs.push(this.events.on("boar:died", () => this.sound.play("crowd")));
      // Play jump sound
