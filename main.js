...
function unlockAudioOnce() {
  ...
  // If SoundManager exists, attempt to start background music now that audio is unlocked.
  try {
    if (soundManager && typeof soundManager.playLoop === "function") {
      // Prefer journey then music
      try {
        if (soundManager.sfx?.journey) soundManager.playLoop("journey");
        else if (soundManager.sfx?.music) soundManager.playLoop("music");
      } catch (e) {}
    }
  } catch (e) {}
}
...
let effects = null; // runtime effects manager (populated in initRuntime)
...
async function boot() {
  ...
  // Load included crowd cheering audio if present in assets root
  try {
    // Auto-register SFX from assets/sfx/ (explicit paths known at edit-time)
    ...
    // Also register the crowd cheer and any 'Journey' music found in assets root
    soundManager.load("crowd", "assets/sfx/crowd.mp3");
    soundManager.load("journey", "assets/sfx/journey.mp3");
  } catch (e) {}
...
function initRuntime() {
  ...
  // -----------------------
  // Simple particle effects (world-triggered, drawn in screen-space)
  // -----------------------
  effects = {
    particles: [],
    spawnBurst(worldX, worldY, color = [255, 200, 80], count = 8) {
    ...
    // draw onto screen-space while camera is OFF (so coordinates are pixels)
    drawOnScreen(viewW, viewH) {
      if (!this.particles.length) return;
      push();
      noStroke();
      for (const p of this.particles) {
        fill(p.color[0], p.color[1], p.color[2], map(p.life, 0, 60, 0, 255));
        ellipse(p.screenX ?? p.x, p.screenY ?? p.y, p.r, p.r);
      }
      pop();
    },
  };

  // Wire events to spawn effects (use world coords where available)
  // These listeners use world coordinates; we render them as screen-space inside drawHudFn.
  if (game && game.events) {
    // leaf:collected -> small burst at player
    game.events.on("leaf:collected", () => {
      try {
        const p = game.level.playerCtrl?.sprite;
        if (p) effects.spawnBurst(p.x, p.y, [120, 220, 140], 10);
      } catch (e) {}
    });

    game.events.on("boar:died", (payload) => {
      try {
        const x = payload?.x ?? (game.level.playerCtrl?.sprite?.x ?? 0);
        const y = payload?.y ?? (game.level.playerCtrl?.sprite?.y ?? 0);
        effects.spawnBurst(x, y, [240, 100, 80], 18);
      } catch (e) {}
    });

    game.events.on("player:damaged", () => {
      try {
        const p = game.level.playerCtrl?.sprite;
        if (p) effects.spawnBurst(p.x, p.y, [255, 80, 80], 12);
      } catch (e) {}
    });
    ...
    game.events.on("player:jumped", () => {
      try {
        const p = game.level.playerCtrl?.sprite;
        if (p) effects.spawnBurst(p.x, p.y + 8, [200, 200, 255], 6);
      } catch (e) {}
    });
  }
  ...
  // Start background music if available (prefer 'journey' then 'music')
  try {
    if (soundManager) {
      const sj = soundManager.sfx?.journey;
      const sm = soundManager.sfx?.music;
      const journeyReady = sj ? (typeof sj.isLoaded === "function" ? sj.isLoaded() : true) : false;
      const musicReady = sm ? (typeof sm.isLoaded === "function" ? sm.isLoaded() : true) : false;

      // Stop any previously-started tracks to avoid overlap
      try {
        if (musicReady) soundManager.stop("music");
      } catch (e) {}

      if (journeyReady) {
        console.log("AUDIO: starting journey loop");
        soundManager.playLoop("journey");
      } else if (musicReady) {
        console.log("AUDIO: journey not ready, starting music loop instead");
        soundManager.playLoop("music");
      } else {
        console.log("AUDIO: no music files ready; background synth may play");
      }
    }
  } catch (e) {}
  ...
  game.draw({
    drawHudFn: () => {
      ...
      try {
        ...
        // Update effects (world-space particles) and compute screen positions
        if (effects) {
          effects.update();
          for (const p of effects.particles) {
            // Map world -> screen (camera.x/y are center coordinates)
            p.screenX = p.x - (camera.x || 0) + viewW / 2;
            p.screenY = p.y - (camera.y || 0) + viewH / 2;
          }
          // draw effects directly to screen (still with camera.off())
          effects.drawOnScreen(viewW, viewH);
        }
